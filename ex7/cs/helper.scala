...
object helpers {
   ...
   */
  val fileList = (path: String) => new java.io.File(path).listFiles.filter(_.getName.endsWith(".vm"))
 
  /**
   * 	append to file use it by given 1 time the file name and than in iteration only the string to add in
   */
  def writeTranslatToFile = (file: String, str: String) => appendToFile(new File(file.replace(".vm", ".asm")), str)
